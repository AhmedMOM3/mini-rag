...
from ..VectorDBEnums import PgVectorTableSchemeEnums, PgVectorIndexTypeEnums, PgVectorDistanceMethodEnums, DistanceMethodEnums
...
class PGVectorProvider(VectorDBInterface):
    ...
    async def get_collection_info(self, collection_name: str) -> dict:
        async with self.db_client() as session:
            async with session.begin():
                table_info_tbl = sql_text(f"""
                    SELECT schemaname, tablename, tableowner, tablespace, hasindexes
                    FROM pg_tables
                    WHERE tablename = :collection_name
                """)
                count_sql = sql_text(f"SELECT COUNT(*) FROM {collection_name}")
                table_info = await session.execute(table_info_tbl, {"collection_name":collection_name})
    ...
    async def insert_one(self, collection_name: str,
        ...
        is_collection_existed = await self.is_collection_existed(collection_name=collection_name)
        if not is_collection_existed:
            self.logger.info(f"Can not insert new recored to non existed collection: {collection_name}")   
            return False
    
        if not record_id:
            self.logger.info(f"Can not insert new recored without record_id")   
            return False           
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                await session.execute(insert_sql,{
                    "text": text,
                    "vector": "[" + ",".join([ str(v) for v in vector ]) + "]", #postgres ned it to be like "[1,2,3]" -> string
    ...
    async def insert_many(self, collection_name: str,
        ...
        is_collection_existed = await self.is_collection_existed(collection_name=collection_name)
        if not is_collection_existed:
            self.logger.info(f"Can not insert new recoreds to non existed collection: {collection_name}")   
            return False
            ...
            async with session.begin():
                for i in range(0, len(texts), batch_size):
                    ...
                    for _text, _vector, _metadata, _record_id in zip(batch_texts, batch_vectors, batch_metadata, batch_record_ids):
                        metadata_json=json.dumps(batch_metadata, ensure_ascii=False) if batch_metadata is not None else "{}"
    ...
    async def search_by_vector(self, collection_name: str, vector: list, limit: int) -> List[RetrievedDocument]:
        ...
        is_collection_existed = await self.is_collection_existed(collection_name=collection_name)
        if not is_collection_existed:
            self.logger.info(f"Can not search for recoreds to non existed collection: {collection_name}")   
            return False
        ...
        async with self.db_client() as session:
            async with session.begin():
                search_sql = sql_text(f"SELECT {PgVectorTableSchemeEnums.TEXT.value} as text, "
                                      f"1 - ({PgVectorTableSchemeEnums.VECTOR.value} <=> :vector) as score "  #get the similarity between {PgVectorTableSchemeEnums.VECTOR.value} column and vector
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                create_idx_sql = sql_text(
                            ...
                            f'USING {index_type} ({PgVectorTableSchemeEnums.VECTOR.value} {self.distance_method})' # to create index it need to know index type, (column_name, distance_method)
                            )
